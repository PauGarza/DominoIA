...
%  Josu� Miguel M�ndez S�nchez	   -  182977  %
...
performMyCommand([Layout,Possible,MyHand,OpHand]):-
    ...
    write("Mis extremos son:"),
    write(Extremos),nl,
    jugable(Extremos,MyHand,ListaJugable), nl,
    write("Mis posibles jugadas son:"),
    write(ListaJugable), nl,
    ListaJugable \== [],
    %chooseMove([Layout,Possible,MyHand,OpHand],ListaJugable),
    performMyCommand([Layout,Possible,MyHand,OpHand],ListaJugable);
    write("No hay jugada disponible, hay que robar"),nl,
    size(Possible,Count),
    Count>=1,
...
performMyCommand([Layout, Possible, MyHand,OpHand], [Side1, Side2]):-
    ...
    enemyTurn([NewLayout, Possible, NewHand2,OpHand]).
/*-------------------- Enemy Moves--------------------*/
...
%Aqu� se encuentra el intento fallido de heur�stica
